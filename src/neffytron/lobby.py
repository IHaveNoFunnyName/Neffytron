...
class SimpleView(discord.ui.View):
    def __init__(self, link):
        super().__init__()
        button = discord.ui.Button(label='Working lobby link because discord sucks', style=discord.ButtonStyle.url, url=shorten(link))
...
class Lobby(Cog):
    ...
    async def lobby_link(self, message):
        match = re.search('(steam:\/\/[^\s]*)', message.content)
        if match:
