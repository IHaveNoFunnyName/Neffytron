...
class Set(Generic[StoredObject]):
...
class _Set(Leaf):
    ...
    def _to_store(self, item):
        ...
        for key in item:
            ...
            result[key] = getattr(self._child, key).set(item[key], self._context)
        return result
    ...
    def _from_store(self, item):
        ...
        for key in dir(self._child):
            if not key.startswith('_'):
                if key not in item:
                    raise ValueError('Invalid key ' + key)
                result[key] = getattr(self._child, key).get(item[key], self._context)
        return result
    ...
    def add(self, item):
        self._context.db[((str(self._context.discord.guild.id) + '.') if self._server else '') + self._path.str(True)].insert_one(self._to_store(item))
    
    def delete(self, query):
        self._context.db[((str(self._context.discord.guild.id) + '.') if self._server else '') + self._path.str(True)].delete_many(self._to_store(query))
